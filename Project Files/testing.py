...
def calculate_regression(csv_file, num_vars, ind_var_predict=None):
    ...
    for i in range(int(num_vars)):
        ...
        x_vals = X.iloc[:, i]
        ax.scatter(x_vals, y, color='blue', label='Data Points')

        regression_line =  x_vals * coefficients[i] + model.intercept_
    
        ax.plot(x_vals, regression_line, color='red', label='Regression Line')
